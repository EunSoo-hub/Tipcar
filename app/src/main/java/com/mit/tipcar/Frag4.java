...
//        Toast.makeText(LocationDemoActivity.this, "Reverse Geo-coding : " + result, Toast.LENGTH_SHORT).show();
        ...
        if ( permsRequestCode == PERMISSIONS_REQUEST_CODE && grandResults.length == REQUIRED_PERMISSIONS.length) {
            ...
            if ( check_result ) {
                ...
                if (ActivityCompat.shouldShowRequestPermissionRationale(mActivity, REQUIRED_PERMISSIONS[0])) {
                    ...
                    Toast.makeText(mActivity, "퍼미션이 거부되었습니다. 앱을 다시 실행하여 퍼미션을 허용해주세요.", Toast.LENGTH_LONG).show();
                    mActivity.finish();


                }else {

                    Toast.makeText(mActivity, "퍼미션이 거부되었습니다. 설정(앱 정보)에서 퍼미션을 허용해야 합니다. ", Toast.LENGTH_LONG).show();
    ...
    public String getCurrentAddress(double latitude, double longitude) {
        ...
        try {
            ...
            //네트워크 문제
            Toast.makeText(mActivity, "지오코더 서비스 사용불가", Toast.LENGTH_LONG).show();
            return "지오코더 서비스 사용불가";
        } catch (IllegalArgumentException illegalArgumentException) {
            Toast.makeText(mActivity, "잘못된 GPS 좌표", Toast.LENGTH_LONG).show();
            return "잘못된 GPS 좌표";
        ...
        if (addresses == null || addresses.size() == 0) {
            Toast.makeText(mActivity, "주소 미발견", Toast.LENGTH_LONG).show();
            return "주소 미발견";
